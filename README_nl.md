...
Deze repository is een aangepaste OpenList-fork die gericht is op `.cas` sidecar-workflows.

Extra wijzigingen in deze fork:

- Bij uploads via de opslagdrivers `189Cloud`, `189CloudPC` en `Local` kan automatisch een `.cas`-bestand met dezelfde naam in de doeldirectory worden gegenereerd.
- Met de optie `Delete source` kan het geuploade bronbestand automatisch worden verwijderd nadat het `.cas`-bestand is aangemaakt.
- Deze fork is bedoeld voor workflows waarbij bestanden naar Tianyi Cloud of lokale opslag worden geüpload en het `.cas`-bestand wordt bewaard voor latere distributie, import of snelle overdracht.
- Wanneer je een Docker-image uit de broncode bouwt, gebruik dan een Git-checkout in plaats van een los broncode-archief, omdat `build.sh` versie-informatie uit Git-metadata leest.

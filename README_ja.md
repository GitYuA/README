...
このリポジトリは、`.cas` サイドカーファイルのワークフロー向けに調整された OpenList のカスタムフォークです。

このフォークで追加された変更点:

- `189Cloud`、`189CloudPC`、`Local` ストレージドライバでは、ファイルのアップロード後に同名の `.cas` ファイルを保存先ディレクトリへ自動生成できます。
- `Delete source` オプションを有効にすると、`.cas` ファイル生成後に元ファイルを自動削除できます。
- 天翼雲盤またはローカルストレージへリソースをアップロードし、後続の配布・インポート・秒伝ワークフロー用に `.cas` ファイルを保持したい用途を想定しています。
- ソースコードから Docker イメージをビルドする場合は、`build.sh` が Git のバージョン情報を参照するため、ソース圧縮ファイルではなく `.git` を含む Git チェックアウトを使用してください。
